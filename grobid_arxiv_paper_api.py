...
from grobid_client.grobid_client import GrobidClient # <-- SWITCHED to Grobid
...
# --- Directory Paths ---
PDF_SAVE_DIR = "dataset/grobid_pdf"
GROBID_XML_SAVE_DIR = "dataset/grobid_xml" # <-- CHANGED from MD_SAVE_DIR
METADATA_SAVE_DIR = "dataset/grobid_paper"
...
def process_with_grobid(pdf_path, save_dir):
    """Processes a PDF with Grobid and saves the XML to the specified directory."""
    try:
        # Assuming config is named grobid_config.json as per your logs
        client = GrobidClient(config_path="./grobid_config.json")
        
        # Use the 'output' parameter as intended by the library.
        # This tells the client WHERE to save the file.
        client.process_pdf(
        ...
        expected_xml_filename = pdf_path_obj.stem + ".grobid.tei.xml"
        save_path = Path(save_dir) / expected_xml_filename

        if save_path.exists() and save_path.stat().st_size > 0:
            return str(save_path)  # Success! Return the path.
        else:
            print(f"[Worker] Grobid call for {pdf_path_obj.name} completed, but the output file is missing or empty.")
            return None  # Failure

    except Exception as e:
        # This will catch timeouts and other connection errors
        print(f"[Worker] An exception occurred during Grobid processing for {Path(pdf_path).name}: {e}")
        return None
...
def download_and_process_paper(paper_info):
    ...
    try:
        ...
        if pdf_path_str:
            ...
            xml_path_str = process_with_grobid(pdf_path_str, GROBID_XML_SAVE_DIR)
            duration = time.monotonic() - start_time
...
def main():
    ...
    print("--- arXiv Paper Processing Workflow Initialized (Backend: Grobid) ---")
    
    Path(PDF_SAVE_DIR).mkdir(parents=True, exist_ok=True)
    Path(GROBID_XML_SAVE_DIR).mkdir(parents=True, exist_ok=True)
