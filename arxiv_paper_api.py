...
METADATA_SAVE_DIR = "paper_metadata" # <-- NEW DIRECTORY FOR JSON FILES
...
def search_arxiv(category, start_date, end_date, max_results):
    ...
    print(f"Querying arXiv API for category '{category}' from {start_date} to {end_date}.")
    ...
    print(f"Metadata for {len(entries)} papers saved to '{METADATA_SAVE_DIR}' directory.")
...
def download_and_process_paper(paper_info):
    ...
    if pdf_path_str:
        ...
        print(f"[Worker] Download of {paper_id} ({page_count} pages) complete. Starting analysis.")
...
def main():
    ...
    Path(METADATA_SAVE_DIR).mkdir(parents=True, exist_ok=True) # <-- CREATE METADATA DIR
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        future_tasks = {executor.submit(download_and_process_paper, paper): paper for paper in papers_to_process}
        ...
        for future in as_completed(future_tasks):
            try:
                ...
                if result['status'] == 'success':
                    ...
                    print(f"Failure: {status_message} for paper {paper_id}. Took {duration:.2f} seconds.")
            except Exception as exc:
                failure_count += 1
                paper = future_tasks[future]
                print(f"Error: An exception occurred while processing paper {paper['id']}: {exc}")
